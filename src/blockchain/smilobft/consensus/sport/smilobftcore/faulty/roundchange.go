...
func (c *core) handleRoundChange(msg *message, src sport.Fullnode) error {
	...
	if c.waitingForRoundChange && num == int(c.fullnodeSet.MaxFaulty()+1) {
		if cv.Round.Cmp(roundView.Round) < 0 {
			c.sendRoundChange(roundView.Round)
		}
		return nil
	} else if num == int(2*c.fullnodeSet.MaxFaulty()+1) && (c.waitingForRoundChange || cv.Round.Cmp(roundView.Round) < 0) {
