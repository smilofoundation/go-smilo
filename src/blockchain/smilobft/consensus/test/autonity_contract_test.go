...
func TestRemoveFromValidatorsList(t *testing.T) {
	...
	testCase := &testCase{
		...
		finalAssert: func(t *testing.T, validators map[string]*testNode) {
			...
			stateDB, _, err := validators["VE"].service.BlockChain().State()
			if err != nil {
				t.Fatal(err)
			}
...
func TestContractUpgrade_Success(t *testing.T) {
	...
	testCase := &testCase{
		...
		genesisHook: func(g *core.Genesis) *core.Genesis {
			...
			// XXX: size limit increased in order to enable autonity contract upgrade is mandatory
			g.Config.CustomTransactionSizeLimit = 64
			return g
		},
		afterHooks: map[string]hook{
