...
// rewards given.
func (c *Bor) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, vaultState *state.StateDB,
	isVault bool) {
	headerNumber := header.Number.Uint64()
	if headerNumber%c.config.Sprint == 0 {
		cx := chainContext{Chain: chain, Bor: c}
		// check and commit span
		if err := c.checkAndCommitSpan(state, header, cx, vaultState, isVault); err != nil {
			log.Error("Error while committing span", "error", err)
			return
		}

		// commit statees
		if err := c.CommitStates(state, header, cx, vaultState, isVault); err != nil {
...
func (c *Bor) FinalizeAndAssemble(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt, vaultState *state.StateDB, isVault bool) (*types.Block, error) {
	headerNumber := header.Number.Uint64()
	if headerNumber%c.config.Sprint == 0 {
		cx := chainContext{Chain: chain, Bor: c}

		// check and commit span
		err := c.checkAndCommitSpan(state, header, cx, vaultState, isVault)
		if err != nil {
			log.Error("Error while committing span", "error", err)
			return nil, err
		}
...
func (c *Bor) checkAndCommitSpan(
	...
	if c.needToCommitSpan(span, headerNumber) {
		err := c.fetchAndCommitSpan(span.ID+1, state, header, chain, vaultState, isVault)
		return err
	}
	return nil
}
...
func (c *Bor) fetchAndCommitSpan(
	...
	// apply message
	return applyMessage(msg, state, header, c.chainConfig, chain, vaultState, isVault)
}
...
func (c *Bor) CommitStates(
	...
	for _, eventRecord := range eventRecords {
		...
		if err := c.GenesisContractsClient.CommitState(eventRecord, state, header, chain, vaultState, isVault); err != nil {
			return err
		}
		lastStateID++
	}
...
func applyMessage(
	...
	vmenv := vm.NewEVM(context, state, vaultState, chainConfig, vm.Config{})
