...
func TestCore_FinalizeAndAssemble(t *testing.T) {
	t.Run("valid params given, no errors returned", func(t *testing.T) {
		...
		backendMock := NewMockBackend(ctrl)
		backendMock.EXPECT().Finalize(nil, header, nil, nil, nil, nil).
			Return(block, nil)
		...
		b, err := c.Finalize(nil, header, nil, nil, nil, nil)
		if err != nil {
			t.Fatalf("Expected <nil>, got %v", err)
		}
	...
	t.Run("invalid params given, error returned", func(t *testing.T) {
		...
		_, err := c.Finalize(nil, header, nil, nil, nil, nil)
		if err != expected {
			t.Fatalf("Expected %v, got %v", expected, err)
		}
	})
...
func TestCore_Seal(t *testing.T) {
	t.Run("valid params given, no errors returned", func(t *testing.T) {
		...
		block, err := c.Seal(nil, block, nil)
		if err != nil {
			t.Fatalf("Expected <nil>, got %v", err)
		}
	})
	...
	t.Run("invalid params given, error returned", func(t *testing.T) {
		...
		backendMock := NewMockBackend(ctrl)
		backendMock.EXPECT().Seal(nil, block, nil).Return(nil, expected)
