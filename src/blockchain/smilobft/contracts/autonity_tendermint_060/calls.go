...
func DeployContract(abi *abi.ABI, autonityConfig *params.AutonityContractGenesis, evm *vm.EVM) error {
	...
	if len(contractBytecode) == 0 {
		panic("could not DeployContract, len(contractBytecode) == 0")
	}
	...
	_, contractAddress, _, vmerr := evm.Create(vm.AccountRef(Deployer), data, gas, value, false)
	if vmerr != nil {
...
func (ac *Contract) updateAutonityContract(header *types.Header, statedb *state.StateDB, bytecode string, state []byte) error {
	...
	gas := uint64(0xFFFFFFFF)
	value := new(big.Int).SetUint64(0x00)
	_, _, _, vmerr := evm.CreateWithAddress(vm.AccountRef(Deployer), data, gas, value, ContractAddress, false)
	if vmerr != nil {
...
func (ac *Contract) AutonityContractCall(statedb *state.StateDB, header *types.Header, function string, result interface{}, args ...interface{}) error {
	...
	ret, _, vmerr := evm.Call(vm.AccountRef(Deployer), ContractAddress, input, gas, new(big.Int), false)
	if vmerr != nil {
...
func (ac *Contract) callSetMinimumGasPrice(state *state.StateDB, header *types.Header, price *big.Int) error {
	...
	_, _, vmerr := evm.Call(vm.AccountRef(Deployer), ContractAddress, input, gas, price, false)
	if vmerr != nil {
