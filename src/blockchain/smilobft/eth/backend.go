...
// Smilo implements the Ethereum full node service.
type Smilo struct {
	config      *Config
	chainConfig *params.ChainConfig

	// Channel for shutting down the service
	shutdownChan chan bool // Channel for shutting down the Smilo
...
func New(ctx *node.ServiceContext, config *Config) (*Smilo, error) {
	...
	var (
		...
		cacheConfig = &core.CacheConfig{Disabled: config.NoPruning, TrieNodeLimit: config.TrieDirtyCache, TrieTimeLimit: config.TrieTimeout}
	)
...
func (s *Smilo) StartMining(local bool) error {
	eb, err := s.Etherbase()
	if err != nil {
...
func (s *Smilo) StopMining()                        { s.miner.Stop() }
