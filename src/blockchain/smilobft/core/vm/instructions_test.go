...
func testTwoOperandOp(t *testing.T, tests []twoOperandTest, opFn func(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack, isVault bool) ([]byte, error)) {
	var (
...
func TestByteOp(t *testing.T) {
	...
	for _, test := range tests {
		...
		stack.push(val)
		stack.push(th)
		opByte(&pc, evmInterpreter, nil, nil, stack, false)
		actual := stack.pop()
