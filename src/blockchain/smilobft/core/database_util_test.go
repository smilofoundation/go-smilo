...
// Tests that setting the flag for Smilo EIP155 activation read values correctly
func TestIsSmiloEIP155Active(t *testing.T) {
	db := ethdb.NewMemDatabase()

	isSmiloEIP155Active := GetIsSmiloEIP155Activated(db)
	if isSmiloEIP155Active {
		t.Fatal("Smilo EIP155 active read to be set, but wasn't set beforehand")
	}

	dbSet := ethdb.NewMemDatabase()
	WriteSmiloEIP155Activation(dbSet)

	isSmiloEIP155ActiveAfterSetting := GetIsSmiloEIP155Activated(dbSet)
	if !isSmiloEIP155ActiveAfterSetting {
		t.Fatal("Smilo EIP155 active read to be unset, but was set beforehand")
	}
}
