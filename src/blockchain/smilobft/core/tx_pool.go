...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if pool.currentState.GetBalance(from).Cmp(big.NewInt(pool.chainconfig.RequiredMinFunds)) < 0 {
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) {
	...
	if isSmilo && pool.pendingState == nil {
		pool.reset(nil, nil)
	}
	...
	for _, addr := range accounts {
		...
		for _, tx := range list.Forward(pool.currentState.GetNonce(addr)) {
			hash := tx.Hash()
			log.Debug("$$$$$$$$$$$$$$ promoteExecutables, Removed old queued transaction", "hash", hash)
			pool.all.Remove(hash)
			pool.priced.Removed()
		}
		//not smilo node or is smilo node with isGas true will drop transactions
		if !isSmilo || isSmilo && isGas {
			...
			//drop all transactions that are too costly (low balance or out smiloPay)
			drops, _ := list.Filter(costLimit, gasLimit)

			for _, tx := range drops {
				hash := tx.Hash()
				log.Debug("$$$$$$$$$$$$$$ promoteExecutables, Removed unpayable queued transaction", "hash", hash, "costLimit", costLimit, "gasLimit", gasLimit, "blockNum", blockNum, "smiloPayLimit", smiloPayLimit)
				pool.all.Remove(hash)
				pool.priced.Removed()
...
func (pool *TxPool) demoteUnexecutables() {
	...
	for addr, list := range pool.pending {
		...
		drops, invalids := list.Filter(costLimit, gasLimit)
