...
func TestSignQuorumEIP155Public(t *testing.T) {
	...
	require.False(t, signedTx.IsPrivate(), fmt.Sprintf("Public transaction is set to a private transaction v == [%v]", signedTx.data.V))
...
func TestSignQuorumHomesteadEIP155SigningPrivateQuorum(t *testing.T) {
	...
	for i := 0; i < len(keys); i++ {
		...
		require.True(t, signedTx.IsPrivate(), fmt.Sprintf("Expected the transaction to be private [%v]", signedTx.IsPrivate()))
