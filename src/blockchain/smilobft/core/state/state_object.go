...
func (self *stateObject) SetBalance(amount, blockNumber *big.Int) {
	self.UpdateSmiloPay(blockNumber)
...
func (self *stateObject) SetSmiloPay(amount *big.Int) {
	self.db.journal.append(smiloPayChange{
		account: &self.address,
		prev:    new(big.Int).Set(self.data.SmiloPay),
	})
	self.setSmiloPay(amount)
}

func (self *stateObject) UpdateSmiloPay(blockNumber *big.Int) {
	prevsmiloPay := self.data.SmiloPay
	prevblock := self.data.BlockNumber
	smiloPay := CalculateSmiloPay(prevblock, blockNumber, prevsmiloPay, self.data.Balance)
	self.db.journal.append(blockChange{
		account:      &self.address,
		prevSmiloPay: prevsmiloPay,
		prevblock:    prevblock,
	})
	self.setSmiloPayAndBlock(smiloPay, blockNumber)
}
...
func (self *stateObject) setSmiloPay(amount *big.Int) {
	self.data.SmiloPay = amount
}

func (self *stateObject) setSmiloPayAndBlock(amount *big.Int, blockNumber *big.Int) {
	self.data.SmiloPay = amount
...
func (self *stateObject) SmiloPay() *big.Int {
	return self.data.SmiloPay
}
