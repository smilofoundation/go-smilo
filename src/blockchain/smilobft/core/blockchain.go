...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state, vaultState *state.StateDB) (status WriteStatus, err error) {
	...
	return bc.writeBlockWithState(block, receipts, state, vaultState)
}

// writeBlockWithState writes the block and all associated state to the database,
// but is expects the chain mutex to be held.
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB, vaultState *state.StateDB) (status WriteStatus, err error) {
	bc.wg.Add(1)
	defer bc.wg.Done()
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals bool) (int, []interface{}, []*types.Log, error) {
	...
	for ; block != nil && err == nil || err == ErrKnownBlock; block, err = it.next() {
		...
		receipts, vaultReceipts, logs, usedGas, err := bc.processor.Process(block, statedb, vaultState, bc.vmConfig)
		if err != nil {
		...
		substart = time.Now()
		if err := bc.validator.ValidateState(block, parentBlock, statedb, receipts, usedGas); err != nil {
			bc.reportBlock(block, receipts, err)
			atomic.StoreUint32(&followupInterrupt, 1)
			return it.index, events, coalescedLogs, err
		}
		...
		status, err := bc.writeBlockWithState(block, allReceipts, statedb, vaultState)
		if err != nil {
			atomic.StoreUint32(&followupInterrupt, 1)
			return it.index, events, coalescedLogs, err
		}
