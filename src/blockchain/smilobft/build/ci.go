...
var (
	...
	allToolsArchiveFiles = []string{
		"COPYING",
		executablePath("sabigen"),
		executablePath("sbootnode"),
		executablePath("sevm"),
		executablePath("sgeth"),
		executablePath("spuppeth"),
		executablePath("srlpdump"),
		executablePath("swnode"),
		executablePath("sclef"),
	}
...
func downloadLinter(cachedir string) string {
	...
	csdb := build.MustLoadChecksums("src/blockchain/smilobft/build/checksums.txt")
...
func doDebianSource(cmdline []string) {
	var (
		goversion = flag.String("goversion", "1.15.5", `Go version to build with (will be included in the source package)`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	if key := getenvBase64("PPA_SIGNING_KEY"); len(key) > 0 {
		...
		fmt.Println("SKIP Import the signing key.")
	}
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Smilo Developers <smilodevs@gmail.com>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	if err := cp.CopyFile(filepath.Join(*workdir, "SimpleFC.dll"), "src/blockchain/smilobft/build/nsis.simplefc.dll"); err != nil {
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "go-smilo/src/blockchain/smilobft/mobile")
